(function () {
  ...
  function renderSlip(calc) {
    ...
    const detail = (label, value) => `<div class="employee-detail"><small>${label}</small><strong>: ${escape(value || "Not available")}</strong></div>`;
    ...
    $("payslipPreview").innerHTML = `<div class="slip-head"><div><div class="slip-brand">OMKAR RETAIL VENTURES</div><small>Employee salary statement</small></div><div class="slip-label">PAYSLIP<small>${escape(c.period)}</small></div></div><div class="slip-person"><div class="employee-column">${detail("EMPLOYEE NAME", employee.name)}${detail("EMPLOYEE ID", employee.id)}${detail("LOCATION", c.location)}${detail("DESIGNATION", employee.role)}${detail("PAID DAYS", `${c.paidDays} / ${c.cycleDays}`)}</div><div class="employee-column">${detail("PAN", employee.pan)}${detail("UAN", employee.uan)}${detail("DATE OF JOINING", displayDate(employee.doj))}${detail("BANK NAME", employee.bank)}${detail("BANK ACCOUNT NUMBER", employee.accountNumber)}${detail("PF NUMBER", employee.pfNumber)}</div></div><div class="slip-tables"><div><span class="earning-head">EARNINGS</span>${earnings}${total("Gross Earnings", c.gross)}</div><div><span class="earning-head">DEDUCTIONS</span>${deductions}${total("Total Deductions", c.deductions)}</div></div><div class="net-pay"><div><small>NET PAYABLE</small><strong>${money(c.net)}</strong></div><div><small>SALARY CYCLE</small><strong>${escape(c.period)}</strong></div></div><p class="note">This is a system-generated payslip and does not require a signature.</p>`;
  }
